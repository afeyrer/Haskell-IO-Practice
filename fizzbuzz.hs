...
fb x f b = if x `mod` f == 0 && x `mod` b ==0 then "FizzBuzz" else if x `mod` f==0 then "Fizz" else if x `mod` b == 0 then "Buzz" else show x
fizzBuzz n f b = [ (fb x f b) | x<- [1..n] ]
main =  do 
        ...
        buzz <- readLn
        mapM_ putStrLn (fizzBuzz num fizz buzz)
